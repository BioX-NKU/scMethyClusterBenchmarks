...
adata = get_N_clusters(adata, n_clusters, 'louvain')

cluster_key='cell_type'
label_key='louvain' # or 'leiden'
print('Louvain')
...
df = pd.DataFrame({'Louvain-C': adata.obs[label_key]})
df.to_csv('./result/Louvain-C.csv', header=True, index=False)
df = pd.DataFrame({'index': ['ari', 'ami', 'nmi', 'fmi', 'comp', 'homo'], 'value': [ARI, AMI, NMI, FMI, Comp, Homo]})
df.to_csv('./result/Evaluation_Louvain-C.csv', header=True, index=False, sep='\t')
