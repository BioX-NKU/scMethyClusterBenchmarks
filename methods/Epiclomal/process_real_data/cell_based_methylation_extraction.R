...
parser$add_argument("--cell_pattern", type="character", help="The pattern of this dataset")
...
get_cov_data <- function(coverage_file, data_type) {
  ...
  if (data_type == "other"){
    ...
    cov_data[,6] <- cov_data[,6] - cov_data[,5]
    cov_data$chr <- gsub("chr", "", cov_data$chr)
...
cov_file <- Sys.glob(file.path(args$path_cell_data, paste0("*", args$cell_ID, args$cell_pattern)))
#originally ".tsv.gz"
