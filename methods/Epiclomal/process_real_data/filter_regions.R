...
main <- function(mean_meth_file, true_file, outdir, coef_t, mean_diff_threshold) {
	...
	#####debug
        print(order(as.character(true_clone_membership$epigenotype_id)))
	print(redundant_matrix)
	print(dim(redundant_matrix))
	print(mode(redundant_matrix))
	#####
        #originally the , is laid behind
	#originally redundant_matrix <- redundant_matrix[order(as.character(true_clone_membership$epigenotype_id),)]
        redundant_matrix <- redundant_matrix[order(true_clone_membership$epigenotype_id)]
        non_redundant_matrix <- non_redundant_matrix[order(true_clone_membership$epigenotype_id)]
